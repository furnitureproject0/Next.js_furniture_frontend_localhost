...
export default function CompanyAdminCreateAppointmentPage() {
    ...
    const handleConfirmLeave = () => {
        ...
        else router.push("/company-admin/dashboard");
    };
                            ...
                            {!selectedClient ? (
                                <>
                                    <div className="relative">
                                        ...
                                        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t("siteAdmin.searchClient") || "Search by name, email, or phone..."} className="w-full pl-10 pr-4 py-2.5 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 bg-white" />
                                        {isSearching && (
                                    ...
                                    {searchResults.length > 0 && (
                                        <div className="border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-100 max-h-60 overflow-y-auto">
                                            {searchResults.map((client) => (
                                                <button key={client.id} onClick={() => handleSelectClient(client)} className="w-full text-left px-4 py-3 hover:bg-blue-50 transition-colors flex items-center gap-3">
