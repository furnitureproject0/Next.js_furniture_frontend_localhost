...
export default function UserManagement() {
    ...
    useEffect(() => {
        // يمكنك إزالة fetchAllUsers من هنا إذا كنت تفضل الاعتماد على الفلترة من الفرونت إند فقط
        // حيث يتم استدعاؤها مسبقاً في الـ Dashboard
    ...
    // تفعيل الفلترة من الفرونت إند (Front-end Filtering) كحل بديل ودائم
    const filteredUsers = allUsers.filter((user) => {
        const matchesSearch = 
            user.name?.toLowerCase().includes(searchQuery.toLowerCase()) || 
            user.email?.toLowerCase().includes(searchQuery.toLowerCase()) ||
            user.phones?.some(phoneObj => phoneObj.phone?.toLowerCase().includes(searchQuery.toLowerCase())) ||
