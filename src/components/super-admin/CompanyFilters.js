...
export default function CompanyFilters({
	...
	const statuses = [
		{ value: "all", label: t("superAdmin.filters.allStatus") },
		{ value: "active", label: t("superAdmin.status.active") },
		{ value: "inactive", label: t("superAdmin.status.inactive") },
	];

	const translatedServices = getTranslatedServiceTypes(t);
	const types = [
		{ value: "all", label: t("superAdmin.filters.allTypes") },
		{ value: "furniture moving", label: translatedServices.find(s => s.id === "furniture_moving")?.name || "Furniture Moving" },
		{ value: "cleaning", label: translatedServices.find(s => s.id === "cleaning_service")?.name || "Cleaning" },
		...
		<div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
			...
			<div className="relative">
				<select
					...
					className="appearance-none px-3 py-2 sm:px-4 sm:py-3 pr-8 sm:pr-10 bg-white border border-primary-200/60 rounded-lg sm:rounded-xl text-sm sm:text-base text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-300 cursor-pointer w-full sm:w-auto min-w-[140px] sm:min-w-[180px]"
				>
					{types.map((type) => (
