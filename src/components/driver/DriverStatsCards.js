...
const StatCard = ({ title, value, icon, bgColor, iconColor }) => (
	<div className="bg-white/80 backdrop-blur-sm rounded-xl border border-primary-200/60 shadow-lg p-4 sm:p-5 lg:p-6 hover:shadow-xl transition-all duration-300">
		<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
			...
			<div
				className={`w-12 h-12 sm:w-14 sm:h-14 ${bgColor} rounded-lg flex items-center justify-center shadow-md flex-shrink-0`}
			>
				{icon}
			</div>
		</div>
...
export default function DriverStatsCards({ assignments = [] }) {
	...
	return (
		<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
			<StatCard
				title={t("driver.stats.totalAssignments") || "Total Assignments"}
			...
			/>
			<StatCard
				title={t("driver.stats.pending") || "Pending"}
				value={stats.pending}
			...
			/>
			<StatCard
				title={t("driver.stats.accepted") || "Accepted"}
			...
			/>
			<StatCard
				title={t("driver.stats.completed") || "Completed"}
				value={stats.completed}
				bgColor="bg-gradient-to-br from-gray-50 to-gray-100"
				icon={
					<svg
						className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-gray-600"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
