...
export default function TimeRangeSelector({
	...
	return (
		<div className="flex items-center flex-wrap gap-1 sm:gap-1 bg-white/80 backdrop-blur-sm rounded-xl p-1 border border-primary-200/60 shadow-sm w-full sm:w-fit">
			{PERIODS.map((period) => (
				<button
					key={period.value}
					...
					className={`flex-1 sm:flex-none px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg transition-all duration-200 cursor-pointer whitespace-nowrap ${
						selectedPeriod === period.value
							...
							: "text-slate-600/70 hover:text-slate-800 hover:bg-primary-50/50"
					}`}
				>
