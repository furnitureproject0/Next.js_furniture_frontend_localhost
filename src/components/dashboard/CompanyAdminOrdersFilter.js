...
export default function CompanyAdminOrdersFilter({ 
	...
	const [filters, setFilters] = useState({
		...
		teamAssigned: initialFilters.teamAssigned || "all", // all, assigned, unassigned
	...
	// Count by team assignment
	const teamAssignmentCounts = useMemo(() => {
		const assigned = orders.filter(o => 
			o.offer?.assignments && o.offer?.assignments.length > 0
		).length;
		return {
			all: orders.length,
			assigned,
		...
		<div className="space-y-4">
			...
			<div className="flex items-center gap-3">
				...
				>
					...
					{t("companyAdmin.filters.filters") || "Filters"}
					{hasActiveFilters && (
				...
				{hasActiveFilters && (
					...
					>
						{t("companyAdmin.filters.clearAll") || "Clear All"}
					</button>
				)}
			</div>
				...
				<div className="p-4 bg-orange-50/50 rounded-lg border border-orange-200/50 space-y-4">
					...
					<div>
						...
						>
							...
							<option value="assigned">
								{t("companyAdmin.filters.teamAssigned") || "Team Assigned"} ({teamAssignmentCounts.assigned})
							</option>
							<option value="unassigned">
								{t("companyAdmin.filters.teamUnassigned") || "No Team"} ({teamAssignmentCounts.unassigned})
							</option>
						</select>
					</div>
