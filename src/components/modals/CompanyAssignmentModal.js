				...
				<div>
					...
					<div className="flex flex-wrap gap-1.5 sm:gap-2">
						{company.specialties.map((specialty, index) => (
							<span
								key={index}
								className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-primary-100 text-primary-800 text-xs font-medium rounded-full"
							>
								{specialty}
							</span>
						))}
			...
			<div className="relative bg-white rounded-xl sm:rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] sm:max-h-[80vh] flex flex-col">
				...
				<div className="flex-shrink-0 p-4 sm:p-5 lg:p-6 border-t border-primary-100/50 bg-gradient-to-r from-primary-50/50 to-primary-50/50">
					<div className="flex justify-end">
						<button
							onClick={onClose}
							className="w-full sm:w-auto px-4 sm:px-6 py-2 text-xs sm:text-sm text-slate-600 hover:text-slate-800 border border-primary-200/60 rounded-lg hover:bg-white/80 transition-colors font-medium cursor-pointer"
						>
